...
export default function CheckoutPage() {
  ...
  if (orderPlaced) {
    return (
      <div className="min-h-screen bg-surface border-t border-line flex items-center justify-center px-4">
        <div className="max-w-lg w-full text-center">
          ...
          <p className="text-ink-2 mb-2">
            Thank you, <span className="text-ink font-bold">{form.name || "Customer"}</span>!
          </p>
          <p className="text-ink-3 text-sm mb-6">
            Your order has been placed and is being processed.
          </p>

          <div className="bg-surface-2 border border-cyan-500/30 rounded-2xl p-5 mb-8 text-left shadow-[0_0_20px_rgba(0,243,255,0.08)]">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-ink-2">Order ID</span>
              <span className="font-mono font-bold text-cyan-600 dark:text-cyan-400">#{orderId}</span>
            </div>
            <div className="flex justify-between text-sm mb-2">
              <span className="text-ink-2">Amount Paid</span>
              <span className="font-mono font-bold text-ink">৳{grandTotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-ink-2">Est. Delivery</span>
              <span className="font-bold text-ink">3–5 Business Days</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link
              href="/items"
              className="rounded-xl border border-line-strong hover:border-cyan-500/50 px-6 py-3 text-sm font-black text-ink hover:text-cyan-600 dark:hover:text-cyan-400 transition-all uppercase tracking-wider"
            >
              Shop More
            </Link>
          </div>
    ...
    return (
      <div className="min-h-screen bg-surface flex flex-col items-center justify-center px-4 border-t border-line">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-3xl font-extrabold text-ink mb-3">Nothing to Checkout</h1>
          <p className="text-ink-2 mb-8">Add products to your cart first.</p>
          <Link
            href="/items"
        ...
        <form onSubmit={handlePlaceOrder}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              {/* Contact */}
              <div className="bg-surface-2 border border-line rounded-2xl p-6 space-y-4">
                <h2 className="font-bold text-ink text-base flex items-center gap-2">
                  <span className="text-cyan-600 dark:text-cyan-400">01</span> Contact Information
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-surface-3 border border-line-strong text-ink rounded-xl text-sm focus:outline-none focus:border-cyan-500 dark:focus:border-cyan-400 transition-all placeholder-ink-3"
                    />
                  </div>
                  <div>
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-[10px] font-bold text-ink-3 uppercase tracking-wider mb-1.5">Phone Number</label>
                    <input
                      name="phone"
                      type="tel"
                      required
              ...
              <div className="bg-surface-2 border border-line rounded-2xl p-6 space-y-4">
                <h2 className="font-bold text-ink text-base flex items-center gap-2">
                  <span className="text-pink-500 dark:text-pink-400">02</span> Shipping Address
                </h2>
                <div>
                  <label className="block text-[10px] font-bold text-ink-3 uppercase tracking-wider mb-1.5">Street Address</label>
                  <input
                    name="address"
                    type="text"
                    required
                    ...
                    className="w-full px-4 py-3 bg-surface-3 border border-line-strong text-ink rounded-xl text-sm focus:outline-none focus:border-pink-500 transition-all placeholder-ink-3"
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-bold text-ink-3 uppercase tracking-wider mb-1.5">City</label>
                    <input
                      name="city"
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-ink-3 uppercase tracking-wider mb-1.5">ZIP / Postal Code</label>
                    <input
                      name="zip"
                      type="text"
                      required
              ...
              <div className="bg-surface-2 border border-line rounded-2xl p-6 space-y-4">
                <h2 className="font-bold text-ink text-base flex items-center gap-2">
                  <span className="text-purple-600 dark:text-purple-400">03</span> Payment Method
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    >
                      ...
                      <span className="text-xs font-bold text-ink">{method.label}</span>
                    </label>
                  ))}
                </div>
                ...
                {form.payment === "card" && (
                  <div className="mt-4 space-y-3">
                    <input
                      ...
                      className="w-full px-4 py-3 bg-surface-3 border border-line-strong text-ink rounded-xl text-sm focus:outline-none focus:border-purple-500 transition-all placeholder-ink-3"
                      maxLength={19}
                    />
            ...
            <div className="lg:col-span-1">
              <div className="bg-surface-2 border border-cyan-500/30 rounded-2xl p-6 shadow-[0_0_20px_rgba(0,243,255,0.08)] sticky top-24 space-y-5">
                ...
                <div className="space-y-3 max-h-56 overflow-y-auto pr-1">
                  {cartItems.map((item) => (
                    <div key={item.id} className="flex gap-3 items-center">
                      <div className="w-12 h-12 flex-shrink-0 bg-surface-4 border border-line rounded-lg overflow-hidden flex items-center justify-center">
                        <img
                          src={item.image}
                          alt={item.title}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-ink-3">Qty: {item.quantity}</p>
                      </div>
                ...
                <div className="border-t border-line pt-4 space-y-2.5 text-sm">
                  ...
                  <div className="flex justify-between text-ink-2">
                    <span>Tax (10%)</span>
                    <span className="font-mono text-ink">৳{tax.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <Link
                  href="/cart"
                  className="block text-center text-xs font-bold text-ink-3 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors"
                >
                  ← Back to Cart
                </Link>
              </div>
